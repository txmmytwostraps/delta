...
- `bank/` (not committed) the problem bank and route, written by `tools/fetch-site.sh` from the site repo at the commit in `SITE_COMMIT`. Change problems in the site repo, then update `SITE_COMMIT` here.
